...
public class StationSubsetWriterNetcdf extends AbstractStationSubsetWriter {
  ...
  public StationSubsetWriterNetcdf(FeatureDatasetPoint fdPoint, SubsetParams ncssParams, NcssDiskCache ncssDiskCache,
    ...
    AttributeContainerMutable attribs = new AttributeContainerMutable("attribs");
    attribs.addAttribute(new Attribute(CDM.TITLE, "Extracted data from TDS Feature Collection"));
    ...
    this.cfWriter = new WriterCFStationCollection(netcdfResult.getAbsolutePath(), attribs, wantedVariables,
        this.stationFeatureCollection.getTimeUnit(), this.stationFeatureCollection.getAltUnits(),
        CFPointWriterConfig.builder().setFormat(NetcdfFormatWriter.convertToNetcdfFileFormat(version)).build());
  }
